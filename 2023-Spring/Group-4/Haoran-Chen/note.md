...
![image](./images/20230712-01.png)

问题：性能相比不基于LSTM的DQN算法没有明显优势

可能原因：状态空间不包括配置决策，时序信息更适合视频分类任务
...
![image](./images/20230614-02.png)

平均精度对比

![image](./images/20230614-01.png)

2、编写LSTM算法

数据输入：50段视频（400帧）随机分配给多用户（3个），模拟视频流过程

网络结构：seq_num帧图片 → ResNet152提取特征 → LSTM网络 → 输出动作对应奖赏

问题：（原建模） utility = accuracy - w * datasize

精度与传输数据量尺度不匹配，举例：

（图片分辨率 - 识别精度 - 数据量） 96p - 0.60828805 - 4.1KB 对比 640p - 0.9008197 - 78.7KB

（修改建模）阶跃函数形式

VoI(utility)=u(accuracy - threshold) - w * datatsize
