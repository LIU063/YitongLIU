...
+ 在BP算法的迭代过程中加入了MLP去替代其中的计算复杂模块，完成知识驱动方法的求解
	- ![image](./picture/MLP迭代_test.png)
	- ![image](./picture/BP+MLP对比.png)
+ 撰写毕设论文，已经完成前两章
## 2023.4.18周报
### 本周工作进展
+ 对BP算法进行修改，在算法中加入了残差，同时修改了因子图的结构，目前能够跑通10个用户，2个BS，每个用户有12种任务卸载策略的情况。
	- 算法迭代图
	- ![image](./picture/10use2MEC3C4D+更严格的限制+正确.png)
+ 学习GNN与BP算法的结合，学习GNN的代码部分
### 下周工作计划
+ 编写GNN与BP算法结合的代码，利用这个算法再解一次之气的问题，期待能得到性能的提升且能够更加灵活的解决更大规模的场景。

## 2023.4.12周报
### 本周工作进展
+ 调试代码，目前对于以下两种简化之后的场景的BP算法已经跑通结果
    - 3个用户，一个MEC，不同用户之间的选择有MEC计算资源限制，形成一个非凸问题，利用loopy belief propagation算法得到近似解
    - 30个用户，5个MEC，不同用户之间的选择没有约束，利用belief propagation算法得到精确解
    - note:现在加入限制条件之后在结果中有体现，只是得变换一下决策规则，现在第一种场景下的迭代图如下：
    - ![image](./picture/3_iteration2.png)
+ 目前存在的的问题
     - 在上边所述第一种场景之上，再加用户数目和MEC数目之后，得到因子节点的概率分布的数组维数过高，遇到缓存溢出的问题，现在在想办法解决
     - 形成因子图过于复杂之后，BP算法得到的结果现在不能保证正确，还没找到原因

### 下周工作计划
+ 解决更多用户，更多MEC情况下，有MEC计算资源限制情况下的loopy belief propagation算法的近似解
##  2023.4.5周报
### 本周工作进展
+ 学习AI在6G中的应用的相关的magzine
+ 讨论场景建模的细节，并对优化目标进行了小的修改
+ 毕设仿真代码部分
     - 完成因子节点的生成和变量节点的生成部分
     - 完成变量节点函数关系部分
...
+ 观看6G大会
   ...
   - 论坛B天地融合智能组网
    ...
    - 论坛F：通感算架构及关键技术
    	- 融合是6G设计的一大理念：星地融合，通信与感知、计算、智能、数据、安全融合
    	- 6G中架构、空口有一个更深入的通感融合
    	      - 增加了通信与感知融合的资源池，增加了融合的增益
    	- 通信与感知共同部署/通信与感知的分别部署
    	      - 收发天线隔离
