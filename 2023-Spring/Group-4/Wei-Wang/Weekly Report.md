...
本周主要工作是基于之前的模型和构想，编写demo仿真程序，目前正在进行环境和MADDPG算法（基线算法）的调试。
...
消息共享指Sp了解其他Sp部分信息的机制，即 Sp 需要通过消息共享机制了解/评估其他 Sp 的定价策略。 Sp有共享部分消息的动机，目的是调控价格。

对于某个 Sp 如 i 而言，我们假使其邻居 Sp 集合为 $N_{i}$, 意味着 Sp 对集合内的的其他 Sp 的出价信息有兴趣。 定义邻居 Sp 集合 可以简化 Sp 决策的难度。

实现消息共享机制的难点有二，其一在于如何确定邻居Sp；其二在于出于隐私考量，Sp 未必同意共享完整信息（即成本构成）给潜在竞争对手，需要对共享的信息进行处理，使其他 Sp 能评估其出价策略的同时，不知道其具体行为。

我们认为信息价值可以作为衡量出价策略的指标，作用于消息共享机制，以解决上述两个问题。
...
定价阶段，没个Sp作为单独的智能体， 其状态是 Nsp 给出的服务底价和资源容量， 动作是出价和购入资源百分比（占总资源数目）；回报通过累积大时间尺度下服务中所有任务的收益获得。

- 任务部署
  
确定当前回合定价和资源数目后，即为每种服务的具体任务部署，每种任务预置几种部署方案以适应不同资源限制情况，一回合结束后反馈回报到Sp.
。
...
- 如何体现信息价值在其中的作用。
- step 2 中的定价不完全博弈过程种涉及信息共享（消息传递）机制，目前还在阅读相关方向的论文。
