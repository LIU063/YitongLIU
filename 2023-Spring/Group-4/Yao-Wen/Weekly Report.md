...
## 下一步工作计划
改变各参数，得到不同参数改变后的仿真图
1、改变车辆个数，总的功率、带宽、计算资源不变，时延容忍不变，车辆个数由4-20增加，每次增加两辆车，记录收敛后的cost值。
2、横坐标为改变转换指令大小，然后两三个数据包大小不同，取10Mbit和20Mbit,分别跑三种算法的cost,并且观察当计算指令增加到一定时，cost开始明显增加
3、改变时延容忍，离散图，横坐标为0.01，0.1，0.3，0.5，1，纵坐标为cost
4、改变任务所需周期数，纵坐标可拆分为能量消耗和支付成本
