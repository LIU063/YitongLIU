...
相关工作：本文列举了一些之前的负载均衡算法，虽然上述算法可以在一定程度上平衡网络负载，但仍存在局部优化问题（考虑相邻的卫星的拥塞信息或两跳的拥塞信息）。在传输过程中，数据包不能完全绕过拥塞的节点。为了解决这一问题，本文提出的LRES算法，试图避免整个数据传输路径上的拥塞，从而实现接近全局优化和网络负载平衡。    
网络模型：类铱星星座，卫星覆盖地面站通信，使用虚拟拓扑模型。在快照中，网络拓扑被认为是静态的。    
基于扩展链路状态的负载均衡路由算法：首先满足延迟约束：传播时延和排队时延。ISL路径时延和上行/下行链路的延迟之和必须小于服务的最小延迟要求。       
链路拥塞阈值的设立：发生拥塞的剩余时延，路径的总传播时延和剩余时延的比值为，越大，卫星节点的丢包率大，拥塞阈值小；当丢包率小时，拥塞阈值大。链接拥塞状态通知机制：两个部分：第一个部分为状态发现机制，请求消息在最短的两条路径上发送，都拥塞则去剩下的路径发送，返回拥塞状态的值busy。第二个部分：当链路上的队列长度超过拥塞阈值时，状态将通知拥塞链接两端的两个节点的邻居节点。LCN的链路反馈时延很短。重路由机制：拥塞链路的末端节点的相邻结点为卫星1和卫星2，寻找卫星1的上游的其他路径，没有再找上游，一直到达源节点没有其他路径就发往最短路径。    
...
解决问题：网关的集中分布和各种LSN业务（例如物联网业务和数据返回）的较大流量返回，将流量严重收敛到有限的网关区域，同时存在严重的网络拥塞和过多的链路负载。    
...
解决问题：以前用于管理SAGIN的集中式体系结构仍然面临着许多实际问题，因此我们考虑了分布式体系结构，但这也面临着许多实际问题。1)如何突出天基网络和空基网络的存储性能特点。2)SAGIN资源管理流程中的每个请求都不提前知道，而是动态到达。为了减少资源分配的延迟，所提出的算法必须立即处理新的传入请求，而不是一次处理大量的请求。3)网络可能有不同的拓扑结构。支持通用拓扑的能力给SAGIN分配算法带来了额外的挑战。解决这些问题，      
...
由于SAGIN的异构、时变、自组织等特点，集中式资源管理方案极其难以实现。为了解决这个问题，我们首先在每个边缘物理域部署一个具有DRL能力的智能服务器，其次，在一个由SAGIN资源属性构建的环境中对代理进行训练，然后根据训练结果对每个用户的请求制定最优的资源分配策略。本文采用DRL方法对基于分布式架构的SAGIN资源管理进行了综合研究。     
