# 202307013周报
### 1、低轨卫星网络切换场景
超密集低轨卫星网络中由于低轨卫星和地面用户相对的高速运动，给低轨卫星通信网络中的卫星切换带来了巨大的挑战，与地面网络的切换方案不同，卫星切换更加复杂，从用户服务质量的角度出发，加权考虑多个切换标准，保证切换的成功率、通信质量、卫星资源的最大利用率和负载的均衡等，满足不同业务的需求。
* 低轨卫星网络的剩余可用时间可从星历表得到，减少切换次数，可以降低服务阻塞率和切换成本吗？
* 可用信道数目可由卫星广播得到，可以反映负载是否均衡，根据用户密度采用一个自适应的k值，反应对可用信道数目的偏好
* 卫星仰角与信噪比之间的关系有固定公式，根据信噪比可以确定传输速率，时延敏感型业务
* 同时考虑星内波束切换(波束切换和星间切换的区别？）
* 频率复用导致产生干扰
* 优化目标是多个属性的加权，权重如何选择？使用层次分析法，对不同类型的业务（高带宽、低时延等）确认使用不同的权重
* 与GEO结合做多层的异构卫星网络融合切换
### 2、可能的创新点
传统方法只考虑当前状态，难以解决高动态的切换决策优化问题，导致用户频繁但不必要的切换，降低了切换成功率，为了使当前的切换有预测性，考虑可以将学习的方法应用于切换决策
### 3、读卫星接入网相关的几篇论文
# 20230629周报
## A Satellite Handover Strategy Based on the Potential Game in LEO Satellite Networks
### 背景
设计一种智能卫星切换策略对最大限度地减少卫星平均切换次数、降低掉话概率、提高呼叫质量和平衡星座网络负载具有重要意义
### 系统模型
考虑一个基于SDSN架构的低轨卫星星地切换模型。卫星网络和卫星终端拓扑的动态特性，将拓扑划分为n个时隙，在每个时隙拓扑保持不变，因此低轨卫星与移动终端之间的连接关系都用二分图表示。在二分图中，一个顶点集显示所有LEO卫星，另一个显示所有移动终端节点，二分图的边缘显示了LEO卫星和移动终端节点之间的呼叫。单个移动终端的卫星切换有三个基本标准，即剩余可见时间、终端的卫星仰角和可用卫星信道的数量。为了使所有移动终端获得尽可能多的剩余可视时间，减少切换次数，同时确保所有移动终端的呼叫质量，采用博弈论的思想，选择的效用函数由两部分组成，即增益函数和损失函数。增益函数由两个切换标准组成，剩余可见时间和卫星仰角。损失函数由切换请求时间和响应时间组成，时间越长，时间效益越差。  
![image](https://github.com/UNIC-Lab/Weekly-Report/assets/127767983/2c2a867b-5608-4649-97c2-d87c42ec3a25)
### 最大化移动终端效益的切换算法
![image](https://github.com/UNIC-Lab/Weekly-Report/assets/127767983/38bbfe80-ef7b-48c2-be4d-d8086780c7aa)  
第一阶段：  
移动终端根据自身的效用功能选择效益最大的卫星进行切换，如果一些移动端只有一个策略，则用不同的标签标记终端，以便在第二阶段进行处理。  
第二阶段：  
对于一些移动终端，可能没有任何可用的信道或可用的卫星，应为其腾出相应的卫星和信道。  
