...
## 面向内容服务的空地集成6G网络资源分配  
### 1、主要内容  
在空地集成6G网络中提出了一个包含内容源、SAGINE和终端用户三个实体的资源分配系统模型，并将问题表述为三方匹配。加入一些限制条件将三方匹配问题转化成限制性三方匹配问题并开发了面向内容的R-TMSC资源分配(COR^2^A)算法和面向用户的R-TMSC资源分配(UO$$R^2$$A)算法来解决上述问题。
### 2、系统模型
