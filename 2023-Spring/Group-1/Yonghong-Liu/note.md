...
发射机部分是辛有限傅里叶变换（finite symplectic Fourier transform，SFFT）和海森堡变换（Heisenberg transform）的组合。

-SFFT公式：

![image](./github/23.3.29-2.png)

公式中xp[k,l]中的k和l代表了时延多普勒域网格中的横纵坐标，而Xp[n,m]代表了n和m代表了时频域网格中的横纵坐标。SFFT完成了从时频域到延迟多普勒域的转换。
...
### 4.接收机部分
接收机部分是魏格纳变换（Wigner transform）和逆
